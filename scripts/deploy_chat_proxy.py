...
async def main():
    ...
    try:
        ...
        try:
            ...
            key_file_url = await am.get_file(artifact_id='ri-scale/openai-secret', file_path='secret.json')
            ...
            if isinstance(key_file_url, str) and key_file_url.startswith('http'):
                 ...
                 content = await am.read_file(artifact_id='ri-scale/openai-secret', file_path='secret.json')
                 import json
    ...
    async def chat(agent_id, message, history=[], context=None):
        ...
        try:
            ...
            if not agent_id.startswith("hypha-agents/"):
                 try:
                     target_service = await server.get_service(f"hypha-agents/{agent_id}")
                 except:
                     pass
        
        if not target_service:
            return {"text": f"Error: Could not find online agent service for ID: {agent_id}"}
